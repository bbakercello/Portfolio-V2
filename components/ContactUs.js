...
const ContactUs = () => {
  ...
  return (
    ...
    <form ref={form} onSubmit={sendEmail} className='flex flex-col mt-4 pl-3 pr-3'>
      ...
      <input type="text" name="user_name" className='p-1 rounded-sm work text-xs' />
      <label className='font-bold pb-2 pt-2'>Email</label>
      <input type="email" name="user_email" className='p-1 rounded-sm work text-xs'/>
      <label className='font-bold pb-2 pt-2'>Message</label>
      <textarea name="message"className='p-1 rounded-sm h-20 work text-xs' />
      <motion.div whileTap={{ scale: 0.8 }} whileHover={{ scale: 1.2 }} className='flex justify-center pt-1 pb-1 '>
